...
QState Octave (Lab2A *me) {
	switch (Q_SIG(me)) {
		...
		case ENCODER_DOWN: //go down an octave
			...
			return  Q_HANDLED();
		case ENCODER_CLICK:
			if (lowOct < 6) decimateFactor = 4;
			else decimateFactor = 8;
			findNoteFreqs(lowOct, A4Val);
			return Q_TRAN(&Menu);
		case BACK:
			return Q_TRAN(&Menu);
...
QState Histogram(Lab2A *me) {
	switch (Q_SIG(me)) {
		case Q_ENTRY_SIG:
			...
			setFont(SmallFont);
			char maxFBuf[10];
			sprintf(maxFBuf, "%.2f", SAMPLING_FREQ / decimateFactor / 2);
			printText(maxFBuf, 184, 0, 0);
			char binSpaceBuf[5];
			sprintf(binSpaceBuf, "%d", SAMPLES / 4);
			printText(binSpaceBuf, 215, 15, 0);
//			char binSize[10];
//			sprintf(binSize, "%.2f", SAMPLING_FREQ / decimateFactor / SAMPLES);
//			printText(binSize, 190, 30, 0);
			return Q_HANDLED();
		case RUN_FFT:
			runFft(1);
			return Q_HANDLED();
...
float runFft(int drawHist) {
	...
	frequency = FFT_simple(input, SAMPLES, twiddles, drawHist, SAMPLING_FREQ/decimateFactor);
	stream_grabber_start();
