...
/* Setup interrupts and reference to interrupt handler function(s)  */
	...
	status = XTmrCtr_Initialize(&tmr, XPAR_AXI_TIMER_1_DEVICE_ID);

	status = XIntc_Connect(&intc, XPAR_INTC_0_TMRCTR_1_VEC_ID, (XInterruptHandler)TimerHandler, &tmr);
	if (status != XST_SUCCESS){
...
//		XGpio_InterruptDisable(&btns, 1);
		...
		if (buttonPressed == 1) {
			// up button = MODE1
			mode = 1;
			QActive_postISR((QActive *)&AO_Lab2A, BACK);
		}
...
void TimerHandler(void *CallbackRef) {
	...
	QActive_postISR((QActive *)&AO_Lab2A, HIDE_A4);
}
